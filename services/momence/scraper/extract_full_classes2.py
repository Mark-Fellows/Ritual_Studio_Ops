"""
FULL CLASSES EXTRACTOR - Business Overview
============================================
This script identifies and extracts all fitness classes that are completely booked 
(where the number of signups equals the class capacity). It processes the latest 
class data from the Momence system and filters for at-capacity classes only.

Source Files: momence_classes_f_*.csv (latest file by modification date)

Output Files: 
1. momence_full_classes_[timestamp].csv - Dated snapshot of full classes from this run
...
2. momence_fullclasses_log.csv - Cumulative log of all full classes over time
...
Business Use Cases:
- Track which classes are selling out
- Identify popular classes for capacity planning
- Monitor trend of full vs. available classes
- Generate waiting list candidates from at-capacity classes
"""
...
# 3. Output filenames
dated_output_file = f"momence_full_classes_{timestamp_str}.csv"
append_output_file = "Log_files/momence_fullclasses_log.csv"

# 4. Extract fully booked classes
matched_rows = []
headers = []
...
with open(latest_file, encoding='utf-8') as infile:
    ...
    for row in reader:
        ...
        capacity = row.get("Capacity", "").replace("NA", "").strip()
        if signups.isdigit() and capacity.isdigit() and int(signups) == int(capacity):
...
print(f"Extracted {extracted_count} full classes to {dated_output_file}")
print(f"Appended to {append_output_file}")

# Also append a short summary line to the shared batch log
append_to_batch_log(f"Extract_full_classes2 ran, and extracted {extracted_count} full classes")
