...
2026-06-27 | Cover Management | DB-level dedup: added unique expression indexes on whatsapp_messages (fingerprint_dedup) and cover_requests (fingerprint_dedup) matching the application fingerprint logic (lower(sender) + hour_bucket_utc + left(text,120)). Cleaned 26 duplicate whatsapp_messages rows and 20 duplicate cover_requests rows (backfill + historical). cover_processor.py insert functions now catch HTTP 409 from DB constraint and log as dedup hit rather than error. | stage2/cover_processor.py, supabase migration whatsapp_cover_request_dedup_indexes
...
2026-06-22 | Cover Management (legacy) | Fixed cover_dashboard.html showing counts but no cards. Two bugs: (1) the "Future classes only" filter dropped every request with no class_date (`if(!class_date) return false`) — so undated Needs-Review requests vanished; now undated requests are kept (return true). (2) "Clear All Filters" actually RE-ENABLED future-only (set futureOnly:true + re-checked the box) — so "remove all filters" re-applied the hiding filter; clearAdvFilters now sets futureOnly:false and unchecks it, so clearing truly shows everything (past + future + undated). Note: stat boxes still count ALL of a status while the list defaults to future-only, so on load the count can exceed visible cards until you clear/untick. Version v1.3.33 -> v1.3.34. | Ritual_Cover_Management/public/cover_dashboard.html

2026-06-22 | Ritual Studio Ops (merged app) + Supabase | Added an "Email link" button to the teacher header (beside Edit/Delete/Send link; canEditTeachers only). DISABLED + greyed when the teacher has no email on file. It POSTs to a NEW Supabase Edge Function `send-schedule-link` (verify_jwt=true, so only signed-in staff can trigger it), which looks up the teacher's email via the service role and emails their personal read-only cover-schedule link (teacher_schedule.html?t=<id>) through Resend — reusing the existing RESEND_API_KEY + verified ritualstudy.com.au sender that campaign-digest already uses. Fully server-side: no mail client, no confirmation. New fn emailTeacherScheduleLink() + showToast feedback. Build stamp bumped P0-5 -> P0-6. Companion to the WhatsApp "Send link" button and the Concept C teacher_schedule.html (cover repo). | Ritual_Studio_Ops/app/ritual-studio-ops-v2.html, Supabase Edge Function: send-schedule-link (v1, deployed)

2026-06-22 | Ritual Studio Ops (merged app) | Teacher header: added a "Send link" button (next to Edit/Delete, canEditTeachers only) that opens WhatsApp pre-filled with the teacher's number and their personal read-only cover-schedule link (https://ritual-cover-management.pages.dev/teacher_schedule.html?t=<id>) — one click for the coordinator, then Send. Uses t.whatsapp_phone (falls back to t.phone); alerts if no number on file. New fn sendTeacherScheduleLink(). Companion to the Concept C teacher_schedule.html (cover repo) + tools/send_schedule_link.py (bulk/CLI send). Requires teacher_schedule.html to be deployed for the link to resolve. | Ritual_Studio_Ops/app/ritual-studio-ops-v2.html

2026-06-22 | Ritual Cover Management | Concept C (WhatsApp-native teacher flow) — first cut. New read-only mobile schedule view public/teacher_schedule.html (keyed by ?t=<teacher_id>; sections: You're covering / Awaiting confirmation / Open for you; de-dupes per class; ?past=1 to include past for testing/history; reads cover_candidates+cover_requests via anon key, RLS off; "reply in WhatsApp to accept/decline" notice — no action buttons). Phone-based sender resolution helper stage2/phone_resolver.py (maps phone-style WhatsApp senders to teachers via teachers.whatsapp_phone, last-9-digit match; validated 3/5 live phone senders resolve) — integration patch for resolve_teacher_id documented in the module, NOT yet applied to cover_processor.py (pending workspace to verify; Edit-truncation risk on that large file). NOT deployed yet. | public/teacher_schedule.html, stage2/phone_resolver.py

2026-06-22 | Ritual Cover Management | Momence teacher roster synced + daily sync automated. Crawled the Momence teacher list (tools/momence_teacher_list.py, 188/190), backfilled momence_ref by email, scraped all unlinked ids (tools/momence_scrape_ids.py), and loaded via tools/load_momence_new.py (match by ref→email→name; system/placeholder filter; dummy-phone blacklist): 13 existing teachers linked + 51 genuinely-new inserted. Roster 181→232 teachers, 162 with phones, 180 linked to Momence. Resolved a duplicate momence_ref (Caitlyn Turner keeps 426550; spurious second Rose Lamont row un-linked). Wired into an unattended daily run: tools/daily_teacher_sync.py (crawl→scrape-new→link/insert) with guards (abort if crawl<100 = logged out; hold inserts if >25), runner run_teacher_sync.bat, scheduled task setup setup_teacher_sync_task.ps1 ("Ritual - Daily Teacher Sync", daily 07:45, own Chrome on port 9333 / C:\MomenceScrape). Also drafted docs/PROMPT-contact-data-quality-table.md for a Teacher-app data-quality table. | tools/momence_teacher_list.py, tools/momence_scrape_ids.py, tools/load_momence_new.py, tools/daily_teacher_sync.py, run_teacher_sync.bat, setup_teacher_sync_task.ps1, docs/MOMENCE_TEACHER_SYNC.md, docs/PROMPT-contact-data-quality-table.md, CLAUDE.md, Supabase teachers (51 inserts)

2026-06-22 | Ritual Cover Management | Momence teacher scrape/sync foundation. New tools/momence_teacher_scraper.py drives its OWN Chrome (port 9333, profile C:\MomenceScrape, logged into Momence) fully separate from the monitor's Edge:9222/WhatsApp; by-id route (107/181 teachers with momence_ref) validated and extracts phone+roles+tags+pay; email/search route is BROKEN (opens the first teacher regardless of email - name-mismatch guard quarantines those rows); render-settle fix added for Momence's async chip sections. New tools/load_momence_scrape.py loads a reviewed CSV -> teachers (dry-run default, --apply, matches by teacher_id, skips NAME MISMATCH, never blanks). Migration momence_teacher_attrs_2026_06_22 APPLIED to live shared DB (additive/nullable): teachers += momence_roles text[], momence_tags text[], momence_synced_at timestamptz. Also corrected stale Phase 4 status (offer-matching shipped 2026-06-02, not "design only"). New doc captures the edit-form structure for future roles/tags WRITE-BACK and a daily new-teacher sync design (list-crawl -> by-id -> insert/update). | tools/momence_teacher_scraper.py, tools/load_momence_scrape.py, MIGRATION-momence-teacher-attrs-2026-06-22.sql, docs/MOMENCE_TEACHER_SYNC.md, SCHEMA.md, CLAUDE.md, PHASE-4-OFFER-MATCHING-DESIGN.md

2026-06-15 | Ritual Campaigns | v5.2.0 - presentation mode (Present button, big-screen layout, Prev/Next/Esc keyboard nav, Outstanding panel, Spark discussion prompts) + meeting capture (paste Gemini summary, Claude returns structured JSON of proposed section updates / action items / decisions, accept/skip each card) + tidy-up (brand voice moved to system_config so it can be edited without redeploy; sidebar variance dots amber>10% red>25%; Live-campaign signup chip in brief header). Migration: campaign_planning_r5_2_brand_voice_into_system_config (single row, 2,707 chars). Edge Function: campaign-ai v2 with meeting_prompts + meeting_capture actions; brand voice loaded from system_config (60s cache, embedded fallback). Deployed via push to ritual-campaign-planning main (commit 4a60a70). | Ritual marketing/campaign-planning.html, Ritual marketing/Docs/Campaign Planning tool build v5.2.md, Ritual marketing/Docs/release-5-2-progress-log.md, Ritual marketing/Docs/Campaign Planning tool build stages.md, Supabase Edge Function: campaign-ai

2026-06-15 | Ritual Campaigns | v5.1.0 - hygiene pass: rename sb -> sbClient, switch Supabase JS CDN jsdelivr -> esm.sh, detectSessionInUrl false -> true, and convert every sb.from() data query (48 callsites) to direct REST via new {data,error}-shaped sbSel/sbIns/sbUpd/sbDel/sbRpc wrappers around the existing R4 dbGet/dbPost/dbPatch/dbDelete helpers. sbClient.auth/channel/storage stay on the JS client per L-TM-01. _r4Token/_r4Headers renamed to cachedAccessToken/restHeaders. Bootstrap uses a Promise gate (no top-level await) so ReactDOM.createRoot mounts only after sbClient is created. Babel-OK + runtime parse-OK. Deployed via push to ritual-campaign-planning main (commit d0c5f7e). | Ritual marketing/campaign-planning.html, Ritual marketing/Docs/Campaign Planning tool build v5.1.md, Ritual marketing/Docs/campaign-planning-hygiene-progress.md

2026-06-15 | Ritual Campaigns | v5.0.4 - actually land the v5.0.3 sidebar polish (v5.0.3 targeted .sidebar-item* CSS but the JSX uses .campaign-item* - dead class names, so none of the styling applied on the live site). The new rules now target the real class family: avatar-style icon circle (32px, sand bg / bark+gold-light when active), deep+cream active state, sand hover, rounded chips on the active row. Also remove the duplicate action-bar Edit button on empty sections - the section-empty-prompt already provides a "Start Editing" CTA and the second action-bar Edit was redundant. Deployed via push to ritual-campaign-planning main (commit cd2fd47). | Ritual marketing/campaign-planning.html

2026-06-15 | Ritual Campaigns | v5.0.3 - sidebar polish to match Studio Ops v2 (warm-white background, avatar-style icon circles inside each item, dark-deep + cream-text active state, hover->sand, rounded filter input); unified button radius and colours across .btn-primary/.btn-secondary/.btn-approve/.btn-danger (8px radius, subtle hover lift, cream filled secondary); defensive fix for the "checklist disappeared after navigating away and back" report - useChecklists / useSections / useAttachments now read the supabase error and bail without clobbering existing state if a fetch fails (most likely a silent auth/lock failure given the campaign app's pre-hygiene sb.from data queries), and ChecklistPanel does an explicit refresh on campaign_id / section_id change as a belt-and-braces guard. Deployed via push to ritual-campaign-planning main (commit 56f512e). | Ritual marketing/campaign-planning.html

2026-06-15 | Ritual Campaigns | v5.0.2 - match Ritual Studio Ops v2 header styling: add the shared Ritual emblem PNG, switch the wordmark to "RITUAL STUDIOS" (italic gold "Studios"), change the subtitle to "CAMPAIGN PLANNING", inline the user email + role badge (campaign_role first, falls back to system role), add Help and Settings placeholder buttons (alert "coming in a future release") next to Sign Out. Existing notif bell preserved; user-chip dropdown retired. Deployed via push to ritual-campaign-planning main. | Ritual marketing/campaign-planning.html

2026-06-15 | Ritual Campaigns | v5.0.1 - move APP_VERSION display from corner footer to the main header (always visible); add the mandatory version-bump rule to Campaign Planning tool build stages.md (Versioning rule near the top) and to both RSO_Campaigns_Change_Prompt.md and RSO_Campaigns_Hygiene_Prompt.md (Definition of done now includes "APP_VERSION incremented"). Format vMAJOR.MINOR.PATCH Rn - patch is the default. Deployed via push to ritual-campaign-planning main (commit 717beeb). | Ritual marketing/campaign-planning.html, Ritual marketing/Docs/Campaign Planning tool build stages.md, RSO_Campaigns_Change_Prompt.md, Ritual marketing/Docs/RSO_Campaigns_Hygiene_Prompt.md

2026-06-15 | Ritual Campaigns | Release 5 - AI assist (human-in-the-loop): new campaign_ai_log audit table; campaign-ai Edge Function deployed (verify_jwt:true, claude-haiku-4-5; embeds Ritual brand voice; logs every call with token usage); new aiCall + AIResultPanel in campaign-planning.html (v5.0 R5, 192,441 bytes); "AI suggest" button per section, "AI reply" button per lead, "Check brief" consistency check button on every campaign. All output reviewed and sent by a human - no autonomous customer email. Deployed via push to ritual-campaign-planning main (commit bdc4339). | migrations/2026-06-15-campaigns-release5.sql, Ritual marketing/campaign-planning.html, Ritual marketing/Docs/06_campaign_planning_release5.sql, Ritual marketing/Docs/Campaign Planning tool build v5.md, Ritual marketing/Docs/Campaign Planning tool build stages.md, Ritual marketing/Docs/release-5-progress-log.md, Supabase Edge Function: campaign-ai

2026-06-15 | Ritual Campaigns | Release 4 - money & signups: 5 new columns on campaigns (enquiries_owner_id, momence_session_ids, xero_tracking_option, finance_reviewer_id, finance_reviewed_at), new tables campaign_leads + campaign_signup_snapshots, views v_campaign_signups_latest/now, finance-reviewer trigger gate (Approved -> Live), pg_cron campaign_signup_snapshot (6-hourly), new Finance/Leads tabs in campaign-planning.html (v4.0 R4), Rose Lamont seeded as finance_reviewer on the four existing campaigns. Momence direct-API and Xero syncs deferred — Finance tab reads public.momence_sessions instead. Deployed via push to ritual-campaign-planning main (commit 285bb13). | migrations/2026-06-15-campaigns-release4.sql, Ritual marketing/campaign-planning.html, Ritual marketing/Docs/05_campaign_planning_release4.sql, Ritual marketing/Docs/Campaign Planning tool build v4.md, Ritual marketing/Docs/Campaign Planning tool build stages.md, Ritual marketing/Docs/Release 4 implementation plan.md, Ritual marketing/Docs/release-4-progress-log.md

2026-06-15 | Cover Management (schema + backend + legacy dashboard) | Add sender_name to cover_requests: schema migration, backfill from whatsapp_messages (86/119 resolved), cover_processor.py updated to populate on new inserts, cover_dashboard.html raw-msg-box now shows [sender] prefix — preserves original WhatsApp poster even after teacher field is edited | migrations/2026-06-15-sender-name-cover-requests.sql, services/cover/stage2/cover_processor.py, public/cover_dashboard.html
...
2026-06-09 | Cover Management (backend) | Fixed IndentationError in cover_processor.py caused by 3 duplicate trailing lines introduced in commit 03a86aa (2026-06-08 21:48). Crashed al
## 2026-06-07 | Cover Management pipeline (stage2) | Cost fix: stop re-parsing already-stored WhatsApp messages. Moved the whatsapp_messages dedup gate BEFORE the NLP step so Claude+Gemini are only called for genuinely new messages (previously every message in the lookback window was re-parsed on all 13 hourly runs and the result discarded at insert-time dedup). Added dynamic lookback (--since-last-run): scrapes back to 30 min before the last run_status='completed' run, capped/falling back to 24h. run_monitor.bat now passes --since-last-run --hours 24 | stage2/cover_processor.py, run_monitor.bat
